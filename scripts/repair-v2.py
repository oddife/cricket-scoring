...
sidebar_pattern = re.compile(
    r'        <div className="grid min-h-\\[calc\\(100vh-10rem\\)\\] lg:grid-cols-\\[150px_minmax\\(0,1fr\\)\\]">\\n'
    r'          /\\* Sidebar \\/\\*\\/\\n'
    r'          <aside className="hidden border-r border-slate-800 bg-\\[#07182d\\] p-3 text-white lg:flex lg:flex-col \\[color-scheme:dark\\]">.*?'
    r'          <\\/aside>',
    re.S,
)

sidebar_replacement = '''        <div id="live-top" className="grid min-h-[calc(100vh-10rem)] lg:grid-cols-[150px_minmax(0,1fr)]">
          {/* Sidebar */}
...
source_marker = "replace_once(\n'''        <div className=\\\"grid min-h"
new_block = '''source = sidebar_pattern.sub(sidebar_replacement, source, count=1)
