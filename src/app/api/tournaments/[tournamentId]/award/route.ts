...
export async function GET(_request: Request, { params }: RouteContext) {
  try {
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Failed to load tournament award." },
      { status: 400 },
    );
  }
}
...
export async function PUT(request: Request, { params }: RouteContext) {
  try {
    ...
    const membership = await prisma.tournamentTeamPlayer.findFirst({
      where: {
        playerId,
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Failed to save Man of the Series." },
      { status: 400 },
    );
  }
}
