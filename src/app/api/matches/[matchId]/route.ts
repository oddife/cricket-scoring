...
function deriveWinner(match: { teamAId: string; teamBId: string; innings: Array<{ battingTeamId: string; totalRuns: number }> }) {
  const totals = new Map<string, number>([[match.teamAId, 0], [match.teamBId, 0]]);
  for (const innings of match.innings) totals.set(innings.battingTeamId, (totals.get(innings.battingTeamId) ?? 0) + innings.totalRuns);
...
export async function PATCH(request: Request, { params }: RouteContext) {
  try {
    ...
    if (requestedStatus === "COMPLETED") {
      ...
      const updated = await prisma.match.update({ where: { id: matchId }, data: { status: "COMPLETED", result: finalResult, winnerId: finalResult === "NORMAL" ? derivedWinnerId : null } });
      const suggestions = await getMatchAwardSuggestions(matchId);
      await prisma.matchAward.upsert({ where: { matchId }, create: { matchId, suggestedPlayerId: suggestions.suggestedPlayerId }, update: { suggestedPlayerId: suggestions.suggestedPlayerId } });
