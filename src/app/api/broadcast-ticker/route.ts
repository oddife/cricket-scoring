...
const DEFAULT_TICKER = {
  ...
  bottomSlots: DEFAULT_BOTTOM_SLOTS, outputWidth: 1280, outputHeight: 100, bottomHeight: 30,
};
...
async function readSlots() {
  ...
  if (raw?.slots && typeof raw.slots === "object") return { activeSlot: slotNumber(raw.activeSlot), slots: Array.from({ length: SLOT_COUNT }, (_, i) => normalizeTicker(raw.slots[String(i + 1)] ?? DEFAULT_TICKER)) };
  const legacy = normalizeTicker(raw ?? DEFAULT_TICKER);
  return { activeSlot: 1, slots: Array.from({ length: SLOT_COUNT }, (_, i) => i === 0 ? legacy : normalizeTicker(DEFAULT_TICKER)) };
}
...
export async function GET(request: Request) {
  try {
    const url = new URL(request.url); const store = await readSlots(); const requested = url.searchParams.get("slot"); const slot = requested == null ? store.activeSlot : slotNumber(requested);
    const ticker = store.slots[slot - 1]; const teamIds = [ticker.teamAId, ticker.teamBId].filter(Boolean);
    const teams = teamIds.length ? await prisma.team.findMany({ where: { id: { in: teamIds } }, select: { id: true, name: true, shortName: true, logo: true } }) : [];
    return NextResponse.json({ ticker, teams, slot, activeSlot: store.activeSlot, slots: [1, 2, 3, 4, 5] });
...
export async function PUT(request: Request) {
  try {
    const body = await request.json(); const slot = slotNumber(body?.slot); const slotsStore = await readSlots();
    const outputWidth = Math.max(640, Math.min(7680, Number(body?.outputWidth) || 1280)); const outputHeight = Math.max(60, Math.min(2160, Number(body?.outputHeight) || 100));
    const ticker = normalizeTicker({ ...body, outputWidth, outputHeight }); delete ticker.slot;
    slotsStore.slots[slot - 1] = ticker; await writeSlots(slotsStore.slots, slot);
    return NextResponse.json({ success: true, ticker, slot, activeSlot: slot, slots: [1, 2, 3, 4, 5] });
