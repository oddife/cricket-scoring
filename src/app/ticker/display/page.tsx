...
function liveTickerFromMatch(match: MatchDetail, ticker: TickerData): TickerData {
  ...
  const battingStats = (id?: string | null) => {
    ...
    return {
      name: playerName(match.players, id),
      runs: String(playerDeliveries.reduce((sum, delivery) => sum + delivery.runsBat, 0)),
      balls: String(playerDeliveries.filter((delivery) => delivery.isLegal).length),
    };
  };
  ...
  return {
    ...
    bowler: {
      name: playerName(match.players, currentBowlerId),
      figures: `${bowlerOvers}-${bowlerRuns}-${bowlerWickets}`,
    },
    lastSix: lastSix.length ? lastSix : ticker.lastSix,
    venue: match.venue ?? ticker.venue,
    toss: match.tossWinner?.name
      ? `TOSS ${match.tossWinner.name}${match.tossDecision ? ` ${match.tossDecision}` : ""}`
      : ticker.toss,
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        // The editor's live-match refresh updates its local state. The display
        // window cannot see that React state, so it must also read the selected
        // match and build the live ticker directly from the current match data.
        if (matchId) {
          try {
            ...
            // Fall back to the last saved ticker if the live match cannot be fetched.
          }
        }
  ...
  return (
    <main
      style={{
        width: "100vw",
        height: "100vh",
        minWidth: `${data.ticker.outputWidth ?? 1280}px`,
        minHeight: `${data.ticker.outputHeight ?? 100}px`,
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 5, padding: "4px 7px", borderRadius: 5, background: "rgba(0,0,0,.65)", color: "#fff" }}>
          ...
          <button
            ...
            style={{ minWidth: 42, background: "transparent", color: "#fff", border: "1px solid rgba(255,255,255,.25)", borderRadius: 3, padding: "1px 4px", font: "12px Arial, sans-serif", cursor: "pointer" }}
          >
            {refreshSeconds}s <span style={{ fontSize: 9, marginLeft: 3 }}>▼</span>
          </button>
        </div>
        {refreshOpen && (
          <div role="listbox" aria-label="Refresh interval" style={{ position: "absolute", top: "100%", right: 0, marginTop: 3, minWidth: 48, padding: 2, border: "1px solid rgba(255,255,255,.22)", borderRadius: 4, background: "rgba(10,10,10,.78)", backdropFilter: "blur(4px)", boxShadow: "0 4px 12px rgba(0,0,0,.35)" }}>
            {REFRESH_OPTIONS.map((seconds) => (
              <button
                type="button"
                role="option"
                aria-selected={refreshSeconds === seconds}
                key={seconds}
                onClick={() => {
                  setRefreshSeconds(seconds);
                  setRefreshOpen(false);
                }}
                style={{ display: "block", width: "100%", border: 0, borderRadius: 2, background: refreshSeconds === seconds ? "rgba(255,255,255,.18)" : "transparent", color: "#fff", padding: "3px 6px", textAlign: "left", font: "12px Arial, sans-serif", cursor: "pointer" }}
              >
                {seconds}s
              </button>
            ))}
