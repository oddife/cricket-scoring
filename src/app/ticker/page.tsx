...
type MatchDetail = LiveMatch & { tossWinner?: { name?: string | null } | null; tossDecision?: string | null; players: { player: Player; teamId: string }[]; innings: Array<{ status: string; totalRuns: number; wickets: number; legalBalls: number; target?: number | null; runRate?: number | null; economy?: number | null; leadDeficit?: { kind: "NONE" | "TRAIL" | "LEVEL" | "LEAD"; runs: number }; currentStrikerId?: string | null; currentNonStrikerId?: string | null; currentBowlerAId?: string | null; deliveries: Array<{ bowlerId: string; strikerId: string; runsBat: number; runsTotal: number; isLegal: boolean; extraType?: string | null; wicket?: { bowlerId?: string | null } | null }> }> };
type DisplayOptions = { score: boolean; overs: boolean; target: boolean; leadDeficit: boolean; runRate: boolean; economy: boolean; batsmen: boolean; bowler: boolean; lastSix: boolean; toss: boolean; venue: boolean; teamNames: boolean };

const DISPLAY_DEFAULTS: DisplayOptions = { score: true, overs: true, target: true, leadDeficit: false, runRate: false, economy: false, batsmen: true, bowler: true, lastSix: true, toss: false, venue: true, teamNames: true };
...
function snapshotFromMatch(match: MatchDetail): Partial<TickerData> {
  ...
  return {
    ...
    bowler: { name: playerName(match.players, currentBowlerId), figures: (() => { const bowlerDeliveries = deliveries.filter((delivery) => delivery.bowlerId === currentBowlerId); const legalBalls = bowlerDeliveries.filter((delivery) => delivery.isLegal).length; const overs = `${Math.floor(legalBalls / 6)}.${legalBalls % 6}`; const runs = bowlerDeliveries.reduce((sum, delivery) => sum + delivery.runsTotal, 0); const wickets = bowlerDeliveries.filter((delivery) => delivery.wicket?.bowlerId === currentBowlerId).length; return `${overs}-${runs}-${wickets}`; })() },
