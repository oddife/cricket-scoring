...
export default function PublicLiveScorePage({ params }: { params: Promise<{ matchId: string }> }) {
  ...
  const teamLinkClass = "text-emerald-300 underline underline-offset-2 decoration-emerald-400/50 transition hover:text-emerald-200 hover:decoration-emerald-300";
  const playerLinkClass = "font-semibold text-emerald-300 underline underline-offset-2 decoration-emerald-400/50 transition hover:text-emerald-200 hover:decoration-emerald-300";
      ...
      <div className="mx-auto w-full max-w-7xl px-3 py-4 sm:px-5 lg:px-8">
        <header className="rounded-2xl border border-slate-700 bg-[#07182d] p-4 shadow-xl sm:p-6">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div><p className="text-xs font-bold uppercase tracking-[0.22em] text-slate-400">Live Score</p><h1 className="mt-1 text-2xl font-black sm:text-3xl"><Link href={`/score/${matchId}/team/${match.teamA.id}`} className={teamLinkClass}>{match.teamA.name}</Link> <span className="text-slate-500">vs</span> <Link href={`/score/${matchId}/team/${match.teamB.id}`} className={teamLinkClass}>{match.teamB.name}</Link></h1></div>
        ...
        {currentInnings && <>
          <section className="mt-4 grid gap-4 lg:grid-cols-[minmax(0,1fr)_320px]">
            <div className="rounded-2xl border border-slate-700 bg-slate-900 p-5 shadow-xl sm:p-7">
              <div className="flex flex-wrap items-end justify-between gap-5">
                <div><p className="text-sm font-semibold text-slate-400">{battingTeam ? <Link href={`/score/${matchId}/team/${battingTeam.id}`} className={teamLinkClass}>{battingTeam.name}</Link> : "Batting"}</p><div className="mt-1 flex items-baseline gap-2"><span className="text-6xl font-black">{currentInnings.totalRuns}</span><span className="text-3xl font-bold text-slate-500">/</span><span className="text-5xl font-black">{currentInnings.wickets}</span></div><p className="mt-2 text-sm text-slate-400">{formatOvers(currentInnings.legalBalls)} overs</p></div>
                <div className="text-right"><p className="text-xs font-bold uppercase tracking-wide text-slate-500">Bowling</p><p className="mt-1 text-lg font-bold">{bowlingTeam ? <Link href={`/score/${matchId}/team/${bowlingTeam.id}`} className={teamLinkClass}>{bowlingTeam.name}</Link> : "—"}</p>{currentInnings.target != null && <p className="mt-1 text-xs text-slate-500">Target {currentInnings.target}</p>}</div>
              </div>
            </div>
          ...
          <section className="mt-4 grid gap-4 lg:grid-cols-3">
            <div className="rounded-2xl border border-slate-700 bg-slate-900 p-5"><p className="text-xs font-black uppercase tracking-wider text-slate-500">Striker</p><p className="mt-2 text-xl font-black">{striker ? <Link href={`/score/${matchId}/player/${striker.id}`} className={playerLinkClass}>{striker.name}</Link> : "—"} *</p></div>
            <div className="rounded-2xl border border-slate-700 bg-slate-900 p-5"><p className="text-xs font-black uppercase tracking-wider text-slate-500">Non-Striker</p><p className="mt-2 text-xl font-black">{nonStriker ? <Link href={`/score/${matchId}/player/${nonStriker.id}`} className={playerLinkClass}>{nonStriker.name}</Link> : "—"}</p></div>
            <div className="rounded-2xl border border-slate-700 bg-slate-900 p-5"><p className="text-xs font-black uppercase tracking-wider text-slate-500">Current Bowler</p><p className="mt-2 text-xl font-black">{bowler ? <Link href={`/score/${matchId}/player/${bowler.id}`} className={playerLinkClass}>{bowler.name}</Link> : "—"}</p></div>
          </section>
        </>}
        ...
        <section className="mt-4 rounded-2xl border border-slate-700 bg-slate-900 p-5 sm:p-6">
          ...
          <div className="mt-5 grid gap-4 lg:grid-cols-2">{[match.teamA, match.teamB].map((team) => <div key={team.id} className="rounded-xl border border-slate-800 bg-slate-950 p-4"><h3 className="text-lg font-black"><Link href={`/score/${matchId}/team/${team.id}`} className={teamLinkClass}>{team.name}</Link></h3><div className="mt-3 grid gap-2 sm:grid-cols-2">{match.players.filter((entry) => entry.teamId === team.id).map((entry) => <div key={entry.playerId} className="rounded-xl border border-slate-800 bg-slate-900 px-3 py-3"><p><Link href={`/score/${matchId}/player/${entry.playerId}`} className={playerLinkClass}>{entry.player.name}</Link></p>{entry.player.jerseyNumber != null && <p className="mt-1 text-xs text-slate-500">#{entry.player.jerseyNumber}</p>}</div>)}</div></div>)}</div>
        </section>

        <section className="mt-4 rounded-2xl border border-slate-700 bg-slate-900 p-5 sm:p-6"><div className="flex items-center justify-between gap-3"><h2 className="text-xl font-black">Innings Summary</h2><span className="text-xs font-semibold uppercase tracking-wide text-slate-500">{match.innings.length} innings</span></div><div className="mt-4 overflow-x-auto"><table className="w-full min-w-[640px] text-left text-sm"><thead className="border-b border-slate-800 text-xs uppercase tracking-wide text-slate-500"><tr><th className="px-3 py-3">Innings</th><th className="px-3 py-3">Batting</th><th className="px-3 py-3">Score</th><th className="px-3 py-3">Overs</th><th className="px-3 py-3">Status</th></tr></thead><tbody>{match.innings.map((innings) => { const team = getTeam(match, innings.battingTeamId); return <tr key={innings.id} className="border-b border-slate-800/80 last:border-0"><td className="px-3 py-3 font-bold">{innings.inningsNumber}</td><td className="px-3 py-3">{team ? <Link href={`/score/${matchId}/team/${team.id}`} className={teamLinkClass}>{team.name}</Link> : "—"}</td><td className="px-3 py-3 font-black">{innings.totalRuns}/{innings.wickets}</td><td className="px-3 py-3">{formatOvers(innings.legalBalls)}</td><td className="px-3 py-3 text-slate-400">{innings.status}</td></tr>; })}</tbody></table></div></section>
