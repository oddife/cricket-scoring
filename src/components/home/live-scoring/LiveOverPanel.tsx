...
type LiveOverPanelProps = {
  ...
  activeBowlerA: any;
  activeBowlerB: any;
  bowlerBallsThisOver: (bowlerId: string) => number;
};
  ...
  return (
    <div id="live-overs" className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm [color-scheme:dark]">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        {oddFinalOver && (
          <div className="rounded-lg bg-amber-100 px-3 py-2 text-xs font-bold text-amber-800">
            ODD FINAL OVER - ONE BOWLER
          </div>
        )}
      </div>
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        ...
        {currentOverDeliveries.length === 0 && (
          <span className="text-sm text-slate-400">No deliveries yet</span>
        )}
      </div>
      ...
      {doubleMode && !oddFinalOver && (
        <div className="mt-3 grid grid-cols-2 gap-3 text-xs">
          ...
          >
            <b>{activeBowlerA?.name ?? "Bowler A"}</b>
            <span className="float-right">
              {liveBowlerAId ? bowlerBallsThisOver(liveBowlerAId) : 0} balls
            </span>
          </div>

          <div
            className={`rounded-lg px-3 py-2 ${
              liveBowlerId === liveBowlerBId
                ? "bg-emerald-50 text-emerald-700"
                : "bg-slate-50 text-slate-600"
            }`}
          >
            <b>{activeBowlerB?.name ?? "Bowler B"}</b>
            <span className="float-right">
              {liveBowlerBId ? bowlerBallsThisOver(liveBowlerBId) : 0} balls
            </span>
          </div>
        </div>
      )}
