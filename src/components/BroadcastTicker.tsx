...
export default function BroadcastTicker({ ticker, teams, displayOnly = false }: Props) {
  const teamA = teams.find((team) => team.id === ticker.teamAId); const teamB = teams.find((team) => team.id === ticker.teamBId); const display = { ...DEFAULT_DISPLAY, ...(ticker.display ?? {}) };
  ...
  const teamAName = ticker.teamAName?.trim() || teamA?.shortName || teamA?.name || "TEAM A"; const teamBName = ticker.teamBName?.trim() || teamB?.shortName || teamB?.name || "TEAM B";
  const width = Math.max(640, Number(ticker.outputWidth ?? 1280)); const height = Math.max(60, Number(ticker.outputHeight ?? 150));
  ...
  return <div className={`${styles["broadcast-stage"]} ${displayOnly ? styles["broadcast-display"] : ""}`} style={{ width: `${width}px`, height: `${height}px`, fontFamily: ticker.font || "Arial, Helvetica, sans-serif", fontSize: `${scale}em` }}><div className={`${styles["broadcast-strip"]} ${styleClass}`}>
    ...
    {(display.batsmen || (display.toss && ticker.toss)) && <div className={styles["broadcast-bottom-row"]}>
