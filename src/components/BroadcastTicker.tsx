...
export default function BroadcastTicker({ ticker, teams, displayOnly = false }: Props) {
  ...
  return <div className={`${styles["broadcast-stage"]} ${displayOnly ? styles["broadcast-display"] : ""}`} style={{ fontFamily: ticker.font || "Arial, Helvetica, sans-serif", fontSize: `${scale}em` }}>
    <div className={`${styles["broadcast-strip"]} ${stripClass}`}>
      <div className={styles["broadcast-team-block"]}><TeamMark team={teamA} fallback="A" /><div className={styles["broadcast-team-copy"]}>{display.teamNames && <strong>{teamA?.shortName || "TEAM A"}</strong>}<span>{ticker.status || "LIVE"}</span></div></div>
      ...
      <div className={`${styles["broadcast-team-block"]} ${styles["right-team"]}`}><div className={`${styles["broadcast-team-copy"]} ${styles.right}`}>{display.teamNames && <strong>{teamB?.shortName || "TEAM B"}</strong>}{display.toss && <span>{ticker.toss || ""}</span>}</div><TeamMark team={teamB} fallback="B" /></div>
