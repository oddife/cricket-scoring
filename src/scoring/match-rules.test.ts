  ...
  it("sets innings 2 target to first innings score plus one", () => {
    expect(
      calculateTarget({
        ...
        battingTeamId: B,
        bowlingTeamId: A,
        previousInnings: [innings(1, A, 150)],
      }),
    ).toBe(151);
  });
  ...
  it("shows the current batting side's aggregate lead", () => {
    expect(
      calculateFourInningsPosition({
        ...
        battingTeamId: A,
        bowlingTeamId: B,
        currentInningsRuns: 30,
        previousInnings: [
          innings(1, A, 100),
          innings(2, B, 120),
        ],
      }),
    ).toEqual({ kind: "LEAD", runs: 10 });
  });
  ...
  it("shows the current batting side's aggregate deficit", () => {
    expect(
      calculateFourInningsPosition({
        ...
        battingTeamId: A,
        bowlingTeamId: B,
        currentInningsRuns: 10,
        previousInnings: [
          innings(1, A, 100),
          innings(2, B, 120),
        ],
      }),
    ).toEqual({ kind: "TRAIL", runs: 10 });
  });
  ...
  it("shows level aggregates", () => {
    expect(
      calculateFourInningsPosition({
        ...
        battingTeamId: A,
        bowlingTeamId: B,
        currentInningsRuns: 20,
        previousInnings: [
          innings(1, A, 100),
          innings(2, B, 120),
        ],
      }),
    ).toEqual({ kind: "LEVEL", runs: 0 });
  });
});
...
describe("match result", () => {
  it("ends a two-innings chase immediately when target is reached", () => {
    expect(
      decideMatchAfterCompletedInnings({
        ...
        battingTeamId: B,
        bowlingTeamId: A,
        currentInningsRuns: 151,
        previousInnings: [innings(1, A, 150)],
        target: 151,
      }),
    ).toEqual({ completed: true, winnerTeamId: B, tie: false });
  });
  ...
  it("continues to innings 4 when innings 3 is level", () => {
    ...
    ).toEqual({ completed: false, winnerTeamId: null, tie: false });
  });
  ...
  it("records a tie when the final aggregates are equal", () => {
    ...
    ).toEqual({ completed: true, winnerTeamId: null, tie: true });
  });
});
